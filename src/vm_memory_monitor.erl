...
-export([update/0, get_total_memory/0, get_vm_limit/0,
		 ...
		 get_vm_memory_low_watermark/0, set_vm_memory_low_watermark/1,
         get_memory_limit/0]).
...
-record(state, {total_memory,
                memory_limit,
				memory_safe,	% jinni, to prevent too frequent alarm set/clear
                timeout,
                timer,
                alarmed
               }).
...
-spec(start_link/0 :: (float()) -> {'ok', pid()} | {'error', any()}).
-spec(update/0 :: () -> 'ok').
...
get_vm_memory_low_watermark() ->
    gen_server:call(?MODULE, get_vm_memory_low_watermark, infinity).

set_vm_memory_low_watermark(Fraction) ->
    gen_server:call(?MODULE, {set_vm_memory_low_watermark, Fraction},
                    infinity).
...
start_link() ->
    ...
    gen_server:start_link({local, ?SERVER}, ?MODULE, [Props], []).
...
init([Props]) ->
    ...
    MemLimit = get_mem_limit(proplists:get_value(high_watermark, Props, 0.40), TotalMemory),
	MemSafe = get_mem_limit(proplists:get_value(low_watermark, Props, 0.38), TotalMemory),
...
handle_call(get_vm_memory_low_watermark, _From, State) ->
    {reply, State#state.memory_safe / State#state.total_memory, State};
...
internal_update(State = #state { memory_limit = MemLimit,
	...
	AlarmReleased = MemUsed < MemSafe,
    case {Alarmed, NewAlarmed, AlarmReleased} of
        {false, true, _} ->
            emit_update_info(set, MemUsed, MemLimit),
            alarm_handler:set_alarm({{vm_memory_monitor, node()}, []});
        {true, _, true} ->
            emit_update_info(clear, MemUsed, MemLimit),
            alarm_handler:clear_alarm({vm_memory_monitor, node()});
        _ ->
            ok
    end,
